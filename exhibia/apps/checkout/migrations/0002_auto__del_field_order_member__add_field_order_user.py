...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Deleting field 'Order.member'
        db.delete_column('checkout_order', 'member_id')

        # Adding field 'Order.user'
        db.add_column('checkout_order', 'user',
                      self.gf('django.db.models.fields.related.ForeignKey')(default=14, to=orm['auth.User']),
                      keep_default=False)


    def backwards(self, orm):

        # User chose to not deal with backwards NULL issues for 'Order.member'
        raise RuntimeError("Cannot reverse this migration. 'Order.member' and its values cannot be restored.")
        # Deleting field 'Order.user'
        db.delete_column('checkout_order', 'user_id')
