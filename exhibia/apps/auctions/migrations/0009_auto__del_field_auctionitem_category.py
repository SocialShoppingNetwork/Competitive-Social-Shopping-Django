...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Deleting field 'AuctionItem.category'
        db.delete_column('auctions_auctionitem', 'category_id')

        # Adding M2M table for field categories on 'AuctionItem'
        db.create_table('auctions_auctionitem_categories', (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('auctionitem', models.ForeignKey(orm['auctions.auctionitem'], null=False)),
            ('category', models.ForeignKey(orm['auctions.category'], null=False))
        ))
        db.create_unique('auctions_auctionitem_categories', ['auctionitem_id', 'category_id'])


    def backwards(self, orm):
        # Adding field 'AuctionItem.category'
        db.add_column('auctions_auctionitem', 'category',
                      self.gf('django.db.models.fields.related.ForeignKey')(to=orm['auctions.Category'], null=True, blank=True),
                      keep_default=False)

        # Removing M2M table for field categories on 'AuctionItem'
        db.delete_table('auctions_auctionitem_categories')
